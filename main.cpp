...
int main()
{
    ...
    for(int y = 0;y < 2;y++){
        ...
        auto seo = std::make_shared<SEO>();
        seo->setUp(R,Rj,cj_leg4,C,vias,leg4);
    ...
    // トリガ
    sim.addVoltageTrigger(200, &command, 0, 0, 0.004);
    sim.addVoltageTrigger(205, &command, 1, 0, 0.004);
    ...
    while(sim.getTime() < endtime){
        ...
        double rectangularV = getRectangularV(sim.getTime(),0.9*tunnelV(C,4,3,cj_leg4,cj_leg3),20,45);
